...
abstract class Author implements ActiveRecordInterface
{
     ...
     * @return void
     */
    public function resetPartialAuthors($v = true): void
    {
     ...
     * @return void
     */
    public function initAuthors(bool $overrideExisting = true): void
    {
        ...
        $this->collAuthors->setModel('\Book');
    }
        ...
        if (null === $this->collAuthors || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collAuthors) {
                    ...
                    $collAuthors = new $collectionClassName;
                    $collAuthors->setModel('\Book');

                    return $collAuthors;
                }
            } else {
                $collAuthors = ChildBookQuery::create(null, $criteria)
                    ->filterByAuthor($this)
                    ->find($con);
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countAuthors(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collAuthorsPartial && !$this->isNew();
        if (null === $this->collAuthors || null !== $criteria || $partial) {
     ...
     * @return $this The current object (for fluent API support)
     */
    public function addAuthor(ChildBook $l)
    {
        if ($this->collAuthors === null) {
            $this->initAuthors();
            $this->collAuthorsPartial = true;
    ...
    /**
     * @param ChildBook $author The ChildBook object to add.
     */
    protected function doAddAuthor(ChildBook $author): void
    {
    ...
    /**
     * @param ChildBook $author The ChildBook object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeAuthor(ChildBook $author)
    {
     ...
     * been saved, it will retrieve related Authors from storage.
     *
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildBook}> List of ChildBook objects
     */
    public function getAuthorsJoinPublisher(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildBookQuery::create(null, $criteria);
        $query->joinWith('Publisher', $joinBehavior);

        return $this->getAuthors($query, $con);
    }
